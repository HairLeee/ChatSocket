...
{   
    ...
    switch (_type)
    {
        ...
        case iCarouselTypeLinear:
        {
            ...
            CGFloat radius = [self valueForOption:iCarouselOptionRadius withDefault:MAX(220 * spacing / 2.0, 220 * spacing / 2.0 / tan(arc/2.0/count))];
            CGFloat angle = [self valueForOption:iCarouselOptionAngle withDefault:offset / count * arc];
            double ra = radius * cos(angle) - radius;
            _itemWidth = 267.0;
            if (_vertical)
            {
                return CATransform3DTranslate(transform, 0.0, offset * _itemWidth * spacing, ra);
            }
            else
            {
                
                return CATransform3DTranslate(transform, offset * _itemWidth * spacing, 0.0, ra);
            }
        }
        case iCarouselTypeRotary:
        case iCarouselTypeInvertedRotary:
...
- (UIView *)loadViewAtIndex:(NSInteger)index withContainerView:(UIView *)containerView
{
    ...
    {
        view = [_dataSource carousel:self viewForItemAtIndex:index reusingView:nil];
    }
